...
export const GridItem: React.FC<ItemProps> = memo(
  ({ id, positions, containerWidth, containerHeight, columns, rows, gap }) => {
  ...
  (prev, curr) => {
    return (
      prev.columns === curr.columns &&
      prev.containerHeight === curr.containerHeight &&
      prev.containerWidth === curr.containerWidth &&
      prev.gap === curr.gap &&
      prev.rows === curr.rows &&
      prev.id === curr.id &&
